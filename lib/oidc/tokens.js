 ...
 * @param {string} [params.issuer] - Issuer to embed (defaults to OIDC_ISSUER_URL).
 *   For white-label domains, pass the issuer derived from the request host so
 *   the `iss` claim matches the domain the relying party is configured against.
 * @returns {string} Signed JWT ID token
 */
export function generateIdToken({ sub, clientId, scopes, nonce, user, issuer }) {
  const now = Math.floor(Date.now() / 1000);

  const payload = {
    iss: issuer || getIssuerUrl(),
    sub,
    aud: clientId,
    exp: now + 3600,
    iat: now,
