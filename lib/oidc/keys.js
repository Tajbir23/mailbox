...
/**
 * Derive the OIDC issuer from the incoming request's host headers.
 *
 * This enables white-label / multi-domain support: each custom domain acts
 * as its own OIDC issuer. The relying party configures the discovery URL on
 * the customer's domain, so the issuer in the discovery document and in
 * issued ID tokens must match that same domain.
 *
 * Falls back to OIDC_ISSUER_URL when no host header is present.
 *
 * @param {Headers} headers - The incoming request headers
 * @returns {string} Issuer origin (e.g. "https://securegptpoint.store")
 */
function getIssuerFromHeaders(headers) {
  const host =
