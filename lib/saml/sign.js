...
function signAssertion(responseXml, assertionId) {
  ...
  // xml-crypto v6 emits the KeyInfo via getKeyInfoContent, passing the chosen
  // signature prefix so the X509 elements share the same (ds:) namespace.
  sig.getKeyInfoContent = (opts) => {
    const prefix = opts && opts.prefix ? `${opts.prefix}:` : "";
    return `<${prefix}X509Data><${prefix}X509Certificate>${certDerBase64}</${prefix}X509Certificate></${prefix}X509Data>`;
  };

  // Place the Signature immediately after the Assertion's Issuer element
  // (the SAML-schema-required position) and emit it with the conventional
  // `ds:` namespace prefix that strict SPs (ChatGPT/WorkOS) expect.
