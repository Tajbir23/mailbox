...
// Manage owner-set tags for a mailbox. Supports comma-separated bulk-add,
// click-to-edit, and per-tag remove. Distinct from per-email tags.
export default function MailboxTagModal({ mailbox, onClose, onUpdated }) {
  ...
  useEffect(() => {
    ...
    setEditing(null);
    setEditText("");
  }, [mailbox?._id]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const persist = async (next) => {
    setBusy(true);
    try {
      const res = await fetch(`/api/mailboxes/${mailbox._id}`, {
        method: "PATCH",
      ...
      setTags(data.tags);
      onUpdated?.(mailbox._id, { tags: data.tags });
      return true;
    } catch (err) {
  ...
  const handleAdd = async (e) => {
    ...
    const ok = await persist([...tags, ...additions]);
    if (ok) setInput("");
  };
  ...
  return (
    <Modal
      ...
      title="Manage Mailbox Tags"
      description={mailbox.emailAddress}
      icon={
    ...
    >
      <div className="space-y-4">
                ...
                editing === tag ? (
                  <li key={tag} className="w-full">
                    <form onSubmit={saveEdit} className="flex items-center gap-2">
                      ...
                      />
                      <button type="submit" disabled={busy} className="text-xs px-3 py-1 rounded-md bg-red-500 hover:bg-red-600 text-white font-medium disabled:opacity-50">Save</button>
                      <button type="button" onClick={cancelEdit} disabled={busy} className="text-xs px-3 py-1 rounded-md hover:bg-surface-100 text-surface-500 font-medium disabled:opacity-50">Cancel</button>
                    </form>
                  </li>
                ) : (
                  <li key={tag}>
        ...
        {/* Email-aggregated tags as info — read-only */}
        {mailbox.emailTags && mailbox.emailTags.length > 0 && (
          <div className="pt-3 border-t border-surface-100">
            <p className="text-[10px] uppercase tracking-wider text-surface-500 font-semibold mb-2">
              Tags from this mailbox&apos;s emails (read-only)
            </p>
            <div className="flex flex-wrap gap-1.5">
              {mailbox.emailTags.map((t) => (
                <span
                  key={t}
                  className="inline-flex items-center px-2 py-0.5 text-xs font-medium text-red-600 bg-white border border-red-200 rounded-md"
                >
                  {t}
                </span>
              ))}
